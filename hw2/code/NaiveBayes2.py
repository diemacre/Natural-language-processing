...
class NaiveBayes:
    ...
    # Recall = TP / (TP+FN)
    def EvalRecall(self, test, Y_pred):
        TP = 0
        FN = 0
...
#    print("Test Accuracy: ", nb.EvalProbabilities(testdata, indexes))
    ...
    print(threshold)
    indexes = []
    for i in range(0, testdata.X.shape[0]):
            indexes.append(i)
    for i in range(0, len(threshold)):
        Y_pred = nb.PredictProbThreshold(testdata, indexes, threshold)
        precisions.append(nb.EvalPrecision(testdata, Y_pred))
        recalls.append(nb.EvalRecall(testdata, Y_pred))
    print("Precisions: ")
    print(precisions)
    print("Recalls: ")
    print(recalls)
